...
const ContactsList = (props) => {
  ...
  useEffect(() => {
    if (nothingSelected) {
      setFilterURL(baseURL + "getFilteredLabels/?pc=0&hc=1&ec=1&rc=1");
    } else {
      setFilterURL(
  ...
  async function fetchContacts(filterURL) {
    try {
      ...
      dispatch({ type: "setContacts", displayedContacts: contactsData });
    } catch (error) {
      console.log(error);
    }
  }
  ...
  let contactsEdited = contacts.map((contact) => ({
    ...
    middleName: getRandom(names),
    lastName: getRandom(names),
    title: getRandom(jobTitles),
    company: getRandom(companyNames),
    type: "Client",
    imageID: getRandom([1, 2, 3, 4, 5]),
    // leadScore: 96,
    // segementType: "Champion",
    ...contact,
  }));

  console.log(contactsEdited);
